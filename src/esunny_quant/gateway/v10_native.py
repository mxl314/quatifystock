...
from .base import EventSink, TradingGateway
...
class V10NativeGateway(TradingGateway):
    ...
    def __init__(self, event_sink: EventSink, config: V10Config) -> None:
        ...
        self.config = config
        self._dll = ctypes.CDLL(str(config.bridge_library.resolve()))
    ...
    def _configure_signatures(self) -> None:
        ...
        d.es_insert_order.argtypes = [ctypes.c_void_p, ctypes.c_char_p, ctypes.c_uint, ctypes.c_char,
            ctypes.c_char, ctypes.c_char, ctypes.c_char, ctypes.c_char, ctypes.c_uint, ctypes.c_uint,
            ctypes.c_double, ctypes.c_uint, ctypes.c_longlong, ctypes.c_uint]
        d.es_insert_order.restype = ctypes.c_int
        d.es_cancel_order.argtypes = [ctypes.c_void_p, ctypes.c_ulonglong, ctypes.c_char_p,
            ctypes.c_uint, ctypes.c_longlong, ctypes.c_uint]
    ...
    def connect(self) -> None:
        ...
        rc = self._dll.es_connect(
            self._handle, self._b(self.config.front_ip), self._b(self.config.account),
            self._b(self.config.password), self._b(self.config.app_id), self._b(self.config.license_no),
            self._b(self.config.log_path.resolve()), self.config.front_port,
        )
        self._check(rc, "连接")
    ...
    def _assert_live(self) -> None:
        if not self.config.live_trading or os.getenv("ESUNNY_LIVE_CONFIRM") != "I_UNDERSTAND":
            raise PermissionError("真实发单被锁定：需 live_trading=true 且 ESUNNY_LIVE_CONFIRM=I_UNDERSTAND")
    ...
    def send_order(self, request_id: int, order: OrderRequest) -> None:
        ...
        rc = self._dll.es_insert_order(
            self._handle, self._b(order.contract), order.contract_index, order.side.value.encode(),
            ...
            request_id, reference, order.seat_index,
        )
        self._check(rc, "报单")

    def cancel_order(self, request_id: int, order_id: int, system_no: str = "") -> None:
        self._assert_live()
        rc = self._dll.es_cancel_order(self._handle, order_id, self._b(system_no), request_id, request_id, 0)
        self._check(rc, "撤单")
